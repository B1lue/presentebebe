...
function App() {
  ...
  // Bucket List States
  const [bucketListVisible, setBucketListVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    if (bucketListRef.current) {
      observer.observe(bucketListRef.current);
    }

    return () => {
      if (bucketListRef.current) {
        observer.unobserve(bucketListRef.current);
      }
    };
  }, []);
